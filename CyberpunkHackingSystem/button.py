...
class Button:
    def __init__(self, text, plane_size, matrix_len, l, c):
        ...
        self.text = self.font.render(text, True, self.ds.button_text_color) # create a text with font propreties and string text
     
    def draw(self, screen):
        pygame.draw.rect(screen, self.ds.button_hitbox_color, self.hitbox) # draw a rectangle (button background) on the screen 
        screen.blit(self.text, self.text.get_rect(center = self.hitbox.center)) # add the text button on the screen positiong at the center of the button

    def draw_click(self, screen):
        pygame.draw.rect(screen, self.ds.button_over_color, self.hitbox, 2) # the 2 allows to put the color an a 2 size board instead of the backgroung
